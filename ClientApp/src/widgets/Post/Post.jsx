...
import { useEditPostMutation, useGetPostQuery, useLazyGetPostQuery, useLazyGetPostsQuery } from '../features/api/apiSlice';
...
const Post = () => {
  ...
  const [deletePost, {data:DeleteSuccess, error:DeletePostError}] = useDeletePostMutation();
  ...
  const { data: post,
    ...
    isSuccess,
    isError,
    error } = useGetPostQuery({id, token});
...
},[isLoading, isSuccess, isError, EditPostError, DeletePostError, DeleteSuccess]);
  //#region EditPost  
  ...
  const sendPost = async() => {
    ...
    await editPost({formData, token, id}).then(()=>{
      getPost({id, token})
  }).catch((error)=>{
      alert(error.message);
    if (error.response.status==401){
